...
describe("Planning (Control Room, native)", () => {
  ...
  it("attaching an existing RH employee auto-assigns offset/binome by creation rank (per room)", async () => {
    await resetPlanningTables();
    const roomA = await createRoom("Salle A");
    const roomB = await createRoom("Salle A-bis");
    const [e1, e2, e3, e4] = employees;

    const a1 = await attachAgent(e1.id, roomA.id, "rotation");
    const a2 = await attachAgent(e2.id, roomA.id, "rotation");
    // Second room: rank restarts at 0 — offsets/binômes are scoped per room.
    const b1 = await attachAgent(e3.id, roomB.id, "rotation");
    const b2 = await attachAgent(e4.id, roomB.id, "rotation");

    expect([a1.offset, a1.binome]).toEqual([0, 1]);
    expect([a2.offset, a2.binome]).toEqual([0, 1]);
    expect([b1.offset, b1.binome]).toEqual([0, 1]);
